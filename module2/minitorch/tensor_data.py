...
def to_index(ordinal, shape, out_index):
    ...
    for i in range(len(out_index)):
        out_index[-1 - i] = ordinal % shape[-1 - i]
        ordinal = int(ordinal / shape[-1 - i])
    # print(out_index)
...
def broadcast_index(big_index, big_shape, shape, out_index):
    ...
    Rets:
        None : Fills in `out_index`.
    """
    # Look at the last [dimensions of shape] dimensions of input
    # If equal then select value from input shape, otherwise set to 0
    # shape = array([0] * len(out_index))
...
def shape_broadcast(shape1, shape2):
    ...
    for index, val in enumerate(shape2_new):
        if (shape1[index] == val) or (shape1[index] == 1) or (val == 1):
            final.append(max(val, shape1[index]))
        else:
...
class TensorData:
    ...
    def permute(self, *order):
        ...
        for val in order:
            new_shape.append(self.shape[val])
            new_strides.append(self.strides[val])
